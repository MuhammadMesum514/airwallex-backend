...
intentRouter.post('/create', async (req, res) => {
  const createIntentUrl = `${config.airwallex.clientApiHost}/api/v1/pa/payment_intents/create`;
  try {
...
intentRouter.post('/elements/3ds/:id', bodyParser.urlencoded(),  async (req, res) => {
  const { id } = req.params;
...
intentRouter.post('/:id/confirm', async (req, res) => {
  const { id } = req.params;
